-- =============================================
-- FIX: Permissions Column Data Corruption (CLEAN VERSION)
-- =============================================
-- Script untuk memperbaiki kolom permissions yang rusak di tabel users

-- Step 0: Complete cleanup of existing structures
...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Permissions column does not exist in users table';
    END IF;
END $$;

-- Step 2: Create user_permissions table with correct structure
...
-- Step 3: Create fresh view for user_list with proper JSON permissions
...
-- Step 4: Set default permissions for all existing users
DO $$
DECLARE
    user_record RECORD;
BEGIN
    -- Loop through all users and set permissions
    FOR user_record IN 
...
DO $$
BEGIN
    RAISE NOTICE '=== PERMISSIONS FIX COMPLETED SUCCESSFULLY ===';
    RAISE NOTICE 'Cleaned up old structures completely';
    RAISE NOTICE 'Dropped corrupted permissions column from users table';
    RAISE NOTICE 'Created fresh user_permissions table structure';
    RAISE NOTICE 'Set default permissions for all existing users';
    RAISE NOTICE 'Created new user_list view with proper JSON permissions';
    ...
    RAISE NOTICE 'All users should now have proper JSON permissions!';
END $$;