...
import { ParameterSetting, CcrParameterData, ParameterDataType, CcrDowntimeData } from '../../types';
import { formatDate, formatNumber, calculateDuration, formatDuration } from '../../utils/formatters';
...
const drawReportOnCanvas = (
    ...
    const totalTableHeight = tableHeaderHeight + (rows.length * ROW_HEIGHT) + (Object.keys(footer).length * FOOTER_ROW_HEIGHT);
    ...
    const operatorTableHeight = (operatorData && operatorData.length > 0)
        ? OPERATOR_SPACING + OPERATOR_TITLE_HEIGHT + OPERATOR_HEADER_HEIGHT + (operatorData.length * OPERATOR_ROW_HEIGHT)
        : 0;
    ...
    const siloTableHeight = (siloData && siloData.length > 0)
    ...
    const downtimeTableHeight = (downtimeData && downtimeData.length > 0)
        ? DOWNTIME_SPACING + DOWNTIME_TITLE_HEIGHT + DOWNTIME_HEADER_HEIGHT + (downtimeData.length * DOWNTIME_ROW_HEIGHT)
        : 0;

    const baseHeight = tableStartY + totalTableHeight + operatorTableHeight + siloTableHeight + downtimeTableHeight + PADDING;
    ...
    ctx.fillText(t.shift, PADDING + COL_HOUR_WIDTH + COL_SHIFT_WIDTH / 2, tableStartY + tableHeaderHeight / 2);
    ...
    rows.forEach((row: any, rowIndex: number) => {
        ...
        ctx.textAlign = 'center';
        ctx.fillText(row.hour.toString().padStart(2, '0') + ':00', PADDING + COL_HOUR_WIDTH / 2, currentY + ROW_HEIGHT / 2);
        ctx.fillText(row.shift, PADDING + COL_HOUR_WIDTH + COL_SHIFT_WIDTH / 2, currentY + ROW_HEIGHT / 2);
    ...
    Object.entries(footer).forEach(([statName, values]) => {
        ...
        ctx.fillStyle = styles.tableHeaderText;
        ctx.textAlign = 'right';
        ctx.fillText(statName, PADDING + COL_HOUR_WIDTH + COL_SHIFT_WIDTH - 10, currentY + FOOTER_ROW_HEIGHT / 2);
    ...
    if (operatorData && operatorData.length > 0) {
        ...
        const opCols = { shift: operatorTableWidth * 0.45, name: operatorTableWidth * 0.55 };
        ...
        ctx.fillText(t.name, PADDING + opCols.shift + opCols.name / 2, currentY + OPERATOR_HEADER_HEIGHT / 2);
        ...
        operatorData.forEach((op: any, index: number) => {
            ...
            ctx.fillText(op.name, PADDING + opCols.shift + opCols.name / 2, currentY + OPERATOR_ROW_HEIGHT / 2);
        ...
        ctx.strokeStyle = styles.borderColor;
        ctx.lineWidth = 1;
        ctx.strokeRect(PADDING, operatorTableStartY + OPERATOR_TITLE_HEIGHT, operatorTableWidth, currentY - (operatorTableStartY + OPERATOR_TITLE_HEIGHT));
        ctx.beginPath();
    ...
    if (siloData && siloData.length > 0) {
        ...
        for (let i = 0; i < 3; i++) {
            ctx.fillText(t.empty_space, headerX + subColWidth / 2, currentY + 45, subColWidth - 4); headerX += subColWidth;
            ctx.fillText(t.content, headerX + subColWidth / 2, currentY + 45, subColWidth - 4); headerX += subColWidth;
            ctx.fillText(t.percentage, headerX + subColWidth / 2, currentY + 45, subColWidth - 4); headerX += subColWidth;
        }
        ...
        siloData.forEach((sd: any, index: number) => {
            ...
            ctx.textAlign = 'left';
            ctx.fillText(sd.master.silo_name, PADDING + 15, currentY + SILO_ROW_HEIGHT / 2, colSiloNameWidth - 20);
            ...
            ['shift1', 'shift2', 'shift3'].forEach(shiftKey => {
                ...
                const percentage = (capacity > 0 && typeof content === 'number') ? (content / capacity * 100) : 0;
                
                ctx.fillText(shiftData?.emptySpace?.toLocaleString('de-DE') || '-', cellX + subColWidth / 2, currentY + SILO_ROW_HEIGHT / 2); cellX += subColWidth;
                ctx.fillText(content?.toLocaleString('de-DE') || '-', cellX + subColWidth / 2, currentY + SILO_ROW_HEIGHT / 2); cellX += subColWidth;
                ctx.fillText(percentage > 0 ? `${percentage.toFixed(1)}%` : '-', cellX + subColWidth / 2, currentY + SILO_ROW_HEIGHT / 2); cellX += subColWidth;
            });
    ...
    if (downtimeData && downtimeData.length > 0) {
        ...
        const dtCols = { start: 100, end: 100, duration: 120, pic: 200, problem: 0 };
        ...
        downtimeData.forEach((d: CcrDowntimeData, index: number) => {
            ...
            ctx.textAlign = 'center';
            // FIX: Use snake_case properties `start_time` and `end_time`
            ctx.fillText(d.start_time, cellX + dtCols.start / 2, currentY + DOWNTIME_ROW_HEIGHT / 2); cellX += dtCols.start;
            ctx.fillText(d.end_time, cellX + dtCols.end / 2, currentY + DOWNTIME_ROW_HEIGHT / 2); cellX += dtCols.end;
            ctx.fillText(durationText, cellX + dtCols.duration / 2, currentY + DOWNTIME_ROW_HEIGHT / 2); cellX += dtCols.duration;
            ctx.fillText(d.pic, cellX + dtCols.pic / 2, currentY + DOWNTIME_ROW_HEIGHT / 2); cellX += dtCols.pic;

            ctx.textAlign = 'left';
            ctx.fillText(d.problem.replace(/\n/g, ' '), cellX + 10, currentY + DOWNTIME_ROW_HEIGHT / 2, dtCols.problem - 20);
        ...
        ctx.strokeStyle = styles.borderColor;
        ctx.lineWidth = 1;
        ctx.strokeRect(PADDING, downtimeTableStartY + DOWNTIME_TITLE_HEIGHT, downtimeTableWidth, currentY - (downtimeTableStartY + DOWNTIME_TITLE_HEIGHT));
        ctx.beginPath();
        let vLineX = PADDING;
...
const ReportPage: React.FC<{ t: any }> = ({ t }) => {
    ...
    const reportConfig = useMemo(() => {
        ...
        const settingsWithDetails = filteredSettings.map(rs => ({
            ...rs,
            // FIX: Use snake_case property `parameter_id`
        ...
        return Object.entries(grouped).map(([category, parameters]) => ({
            category,
            parameters: parameters.sort((a,b) => a.parameter.localeCompare(b.parameter)),
        }));
    ...
    const handleGenerateReport = useCallback(async () => {
        ...
        const filteredSiloData = allSiloDataForDate.filter(data => {
        ...
        .filter((data): data is typeof data & { master: NonNullable<typeof data.master> } => !!data.master);
        ...
        if (operatorParam) {
            ...
            operatorData = [
                { shift: `${t.shift_3} (${t.shift_3_cont})`, name: getOperatorForShift([1,2,3,4,5,6,7]) },
                { shift: t.shift_1, name: getOperatorForShift([8,9,10,11,12,13,14,15]) },
                { shift: t.shift_2, name: getOperatorForShift([16,17,18,19,20,21,22]) },
        ...
        allParams.forEach(param => {
            ...
            if (param.data_type === ParameterDataType.NUMBER) {
                ...
                if (values.length > 0) {
                    footerStats[t.average][param.id] = (values.reduce((a,b) => a+b, 0) / values.length).toLocaleString('de-DE', {maximumFractionDigits: 2});
                    footerStats[t.min][param.id] = Math.min(...values).toLocaleString('de-DE', {maximumFractionDigits: 2});
                    footerStats[t.max][param.id] = Math.max(...values).toLocaleString('de-DE', {maximumFractionDigits: 2});
                }
            }
        });
    ...
    }, [selectedDate, selectedUnit, reportConfig, getDataForDate, getDowntimeForDate, getSiloDataForDate, siloMasterData, t, parameterSettings]);
        ...
        <div className="space-y-6">
            <div className="bg-white p-4 rounded-lg shadow-md">
                <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                    <h2 className="text-xl font-semibold text-slate-800">{t.op_report}</h2>
                    <div className="flex flex-col lg:flex-row items-center gap-4 flex-wrap">
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                            <label htmlFor="report-category" className="text-sm font-medium text-slate-700 whitespace-nowrap">{t.plant_category_label}:</label>
                            <select 
                                id="report-category"
                         ...
                         <div className="flex items-center gap-2 w-full sm:w-auto">
                            <label htmlFor="report-unit" className="text-sm font-medium text-slate-700 whitespace-nowrap">{t.unit_label}:</label>
                            <select 
                                id="report-unit"
                                value={selectedUnit}
                            ...
                            >
                                {unitsForCategory.map(unit => <option key={unit} value={unit}>{unit}</option>)}
                            </select>
                        </div>
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                            <label htmlFor="report-date" className="text-sm font-medium text-slate-700">{t.select_date}:</label>
                            <input
                                type="date"
                                id="report-date"
                                value={selectedDate}
                             ...
                             {reportImageUrl && (
                                <>
                                    <button
                                        onClick={handleCopy}
                                        disabled={isCopied}
                                        className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-semibold text-slate-700 bg-slate-100 rounded-md shadow-sm hover:bg-slate-200 disabled:opacity-75 transition-colors"
                                    >
                                            ...
                                            <div className="absolute right-0 mt-2 w-56 origin-top-right bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
                                                <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                                                    <button
                                                        ...
                                                        className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-slate-900"
                                                        role="menuitem"
                                                    >
                                                        {t.download_as_image}
                                                    </button>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md min-h-[60vh] flex items-center justify-center">
                ...
                {reportConfig.length === 0 && (
                     <div className="text-center text-slate-500">
                        ...
                        <p>Please configure parameters for the selected unit '{selectedUnit}' in Plant Operations - Master Data.</p>
                    </div>
                )}
                {isLoading && (
                    <div className="text-center text-slate-500">
                ...
                {reportImageUrl && !isLoading && (
                    <div className="w-full overflow-x-auto">
                         <img src={reportImageUrl} alt={t.op_report_title} className="max-w-none w-full h-auto" />
                    </div>
                )}
