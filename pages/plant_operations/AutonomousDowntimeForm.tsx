    ...
    <form onSubmit={handleSubmit}>
      <div className="p-6 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <div>
            <strong className="text-slate-600">{t.date}:</strong>{" "}
            <span className="text-slate-800">
              {formatDate(recordToEdit.date)}
            </span>
          </div>
          <div>
            <strong className="text-slate-600">{t.unit}:</strong>{" "}
            <span className="text-slate-800">{recordToEdit.unit}</span>
          </div>
          <div>
            <strong className="text-slate-600">{t.start_time}:</strong>{" "}
            <span className="text-slate-800 font-mono">
              {recordToEdit.start_time}
            </span>
          </div>
          <div>
            <strong className="text-slate-600">{t.end_time}:</strong>{" "}
            <span className="text-slate-800 font-mono">
              {recordToEdit.end_time}
            </span>
          </div>
          <div className="sm:col-span-2">
            <strong className="text-slate-600">{t.problem}:</strong>{" "}
            <span className="text-slate-800">{recordToEdit.problem}</span>
          </div>
        </div>
