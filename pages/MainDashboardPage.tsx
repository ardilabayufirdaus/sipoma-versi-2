    ...
    >
      ...
      <div className="relative">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <H1 className="mb-1 tracking-tight text-xl">Selamat Datang di SIPOMA</H1>
            <Body size="small" className="text-white/90">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className={`
                  flex items-center space-x-2 px-4 py-2 rounded-xl backdrop-blur-sm border border-white/30 transition-all duration-300
                  ${
                    isAutoRefresh
            ...
            <button
              onClick={onRefresh}
              className="p-3 bg-white/20 hover:bg-white/30 backdrop-blur-sm border border-white/30 rounded-xl hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5"
            >
            ...
            {onExport && (
              <button
                onClick={onExport}
                className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 border border-green-500 rounded-xl shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 text-white font-semibold"
              >
    ...
    <>
      <motion.div
        ...
        whileHover={{
          ...
          y: isInteractive ? -4 : 0,
          boxShadow: isInteractive ? '0 20px 40px rgba(0,0,0,0.1)' : undefined,
        }}
...
const QuickLinkCard: React.FC<QuickLinkCardProps> = ({ title, icon, onClick }) => (
  <motion.button
    ...
    transition={{ type: 'spring', stiffness: 400, damping: 17 }}
    onClick={onClick}
    className="w-full glass-card p-3 rounded-xl hover-lift group transition-all duration-300 relative overflow-hidden focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50"
...
const MainDashboardPage: React.FC<MainDashboardPageProps> = ({
  ...
  // Hooks for data (placeholders for future use)
  ...
  const getKPITrend = (currentValue: number) => {
  ...
  if (!hasDashboardAccess) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 overflow-hidden">
      <div className="h-full max-w-screen-2xl mx-auto p-3 space-y-4 flex flex-col">
        {/* Enhanced Header */}
        ...
        <motion.div
          ...
          className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-3 flex-shrink-0"
        >
          <QuickStatCard
            title="Active Users"
            value={usersCount}
          ...
          <QuickStatCard
            ...
            variant="online"
            trend={getKPITrend(onlineUsersCount)}
            onClick={() => onNavigate('users')}
          />
          ...
          <QuickStatCard
            ...
            variant="success"
            trend={getKPITrend(activeProjects)}
        ...
        <motion.div
          ...
          className="bg-white/80 backdrop-blur-xl dark:bg-slate-800/80 rounded-xl border border-slate-200/50 dark:border-slate-700/30 p-3 shadow-lg mt-auto"
        >
