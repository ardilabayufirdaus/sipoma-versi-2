...
import { MetricCard, MetricCardProps } from '../../components/logistics/MetricCards';
...
const LogisticsPerformance: React.FC<PageProps> = ({ t, areas, stockRecords, masterData }) => {
  ...
  const performanceData = useMemo(() => {
    ...
    const relevantMasterData = masterData.filter(
      (m) => debouncedFilterArea === 'All Areas' || m.area === debouncedFilterArea
    );
    const relevantStockRecords = stockRecordsForMonth.filter(
      (r) => debouncedFilterArea === 'All Areas' || r.area === debouncedFilterArea
    );

    const daysInMonth = new Date(debouncedFilterYear, debouncedFilterMonth + 1, 0).getDate();
    ...
    const totalSiloCapacity = relevantMasterData.reduce((sum, r) => sum + r.silo_capacity, 0);
    const totalDeadStock = relevantMasterData.reduce((sum, r) => sum + r.dead_stock, 0);
    const totalLiveStockCapacity = totalSiloCapacity - totalDeadStock;

    const totalStockOut = relevantStockRecords.reduce((sum, r) => sum + r.stock_out, 0);
    const avgDailyStock =
    ...
    const utilization =
      totalLiveStockCapacity > 0 ? (avgDailyStock / totalLiveStockCapacity) * 100 : 0;

    const masterCapacityByArea = masterData.reduce(
      (acc, m) => {
    ...
    // Chart Data by Area
    const chartDataByArea = displayedAreas.reduce(
      (acc, area) => {
    ...
    const comparisonDataByArea = displayedAreas.reduce(
      (acc, area) => {
        const areaRecords = stockRecordsForPreviousMonth.filter((r) => r.area === area);
        const previousDaysInMonth = new Date(previousYear, previousMonth + 1, 0).getDate();
        const previousDaysArray = Array.from({ length: previousDaysInMonth }, (_, i) => i + 1);
        const stockOutByDay = new Map(
          areaRecords.map((r) => [new Date(r.date).getDate(), r.stock_out])
        );
    ...
    stockRecordsForMonth.forEach((r) => {
      if (!stockOutByAreaAndDay.has(r.area)) stockOutByAreaAndDay.set(r.area, new Map());
    ...
    const tableFooter = daysArray.map((day) => {
      return tableRows.reduce((sum, row) => sum + (row.dailyValues[day - 1] || 0), 0);
    });
  ...
  const handleChartHover = (event: React.MouseEvent) => {
    ...
    const values = performanceData.displayedAreas.reduce(
      (acc, area) => {
        const dayData = performanceData.chartData[area]?.find((d) => d.day === day);
        if (dayData) {
    ...
    const xPosInSvg =
      performanceData.daysInMonth > 1 ? (dayIndex / (performanceData.daysInMonth - 1)) * 500 : 250;
  ...
  const metrics: MetricCardProps[] = [
      ...
      breakdownData: {
        ...
        chartData: Array.from({ length: 7 }, (_, i) => ({
          ...
          utilization: Math.max(
            50,
            Math.min(100, performanceData.utilization + (Math.random() - 0.5) * 20)
          ),
          target: 80,
        })),
      ...
      breakdownData: {
        ...
        metrics: [
          ...
          },
          {
            ...
            value: (performanceData.overSlotFrequency / areas.length).toFixed(1),
            unit: 'times',
          },
          {
    ...
    },
    {
      ...
      breakdownData: {
        ...
        description: 'Detailed analysis of days with critical operational issues',
        metrics: [
          {
    ...
    },
    {
      ...
      breakdownData: {
        ...
        metrics: [
          ...
          {
            label: 'Peak Day',
            value: formatNumber(Math.round((performanceData.totalStockOut / 30) * 1.5)),
            unit: 'tons',
          },
          {
        ...
        chartData: Array.from({ length: 7 }, (_, i) => ({
          name: `Week ${i + 1}`,
          stockOut: Math.round((performanceData.totalStockOut / 7) * (0.8 + Math.random() * 0.4)),
          target: Math.round(performanceData.totalStockOut / 7),
        })),
        chartType: 'line' as const,
        details: areas.map((area) => ({
          label: area,
          value: `${formatNumber(Math.round(performanceData.totalStockOut / areas.length))} tons`,
          status: 'neutral',
        })),
      },
    },
      ...
      breakdownData: {
        ...
        metrics: [
          ...
          },
          {
            ...
            value: formatNumber(Math.round(performanceData.totalSiloCapacity / areas.length)),
            unit: 'tons',
          },
          {
            label: 'Largest Silo',
            value: formatNumber(
              Math.round((performanceData.totalSiloCapacity / areas.length) * 1.3)
            ),
            unit: 'tons',
          },
        ],
        chartData: areas.map((area) => ({
          name: area,
          capacity: Math.round(
            (performanceData.totalSiloCapacity / areas.length) * (0.8 + Math.random() * 0.4)
          ),
    ...
    },
    {
      ...
      breakdownData: {
        ...
        description: 'Analysis of non-moving inventory and optimization opportunities',
        metrics: [
          {
          ...
          },
          {
            ...
            value: (
              (performanceData.totalDeadStock / performanceData.totalSiloCapacity) *
              100
            ).toFixed(1),
            unit: '%',
          },
          {
            ...
            value: formatNumber(Math.round(performanceData.totalDeadStock * 500)),
            unit: 'USD',
          },
          {
            label: 'Potential Savings',
            value: formatNumber(Math.round(performanceData.totalDeadStock * 100)),
            unit: 'USD',
          },
        ],
        chartData: areas.map((area) => ({
          ...
          deadStock: Math.round(
            (performanceData.totalDeadStock / areas.length) * (0.5 + Math.random())
          ),
          totalStock: Math.round(performanceData.totalSiloCapacity / areas.length),
        })),
        chartType: 'bar' as const,
        details: [
    ...
    },
    {
      ...
      breakdownData: {
        ...
        metrics: [
          ...
          {
            label: 'Optimal Level',
            value: formatNumber(Math.round(performanceData.avgDailyStock * 1.1)),
            unit: 'tons',
          },
        ],
        chartData: Array.from({ length: 30 }, (_, i) => ({
          name: `Day ${i + 1}`,
          actualStock: Math.round(performanceData.avgDailyStock * (0.8 + Math.random() * 0.4)),
  ...
  const chartInsights = useMemo(() => {
    ...
    const totalDaily =
      performanceData.chartData[performanceData.displayedAreas[0]]?.map((d, index) => {
    ...
    const avgDaily = totalDaily.reduce((sum, val) => sum + val, 0) / totalDaily.length;
    ...
    <div className="space-y-4" aria-live="polite" role="main">
      <div className="bg-white p-4 rounded-lg shadow-md">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-3">
          <h2 id="logistics-title" className="text-lg font-bold text-slate-800 tracking-tight">
      ...
      {loading ? (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
            {Array.from({ length: 8 }, (_, i) => (
              <div key={i} className="bg-white p-3 rounded-lg shadow-md animate-pulse">
        ...
        <>
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg px-4 py-3 mb-6">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-blue-600 dark:text-blue-400">
                {performanceData.displayedAreas.length} area, {performanceData.daysInMonth} hari
              </span>
            </div>
          </div>
          ...
          <ErrorBoundary>
            <div className="bg-white p-4 rounded-lg shadow-md" aria-labelledby="chart-title">
              <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-2 mb-3">
                <div>
                  <h3 id="chart-title" className="text-base font-semibold text-slate-800">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <div className="flex items-center gap-1">
                    ...
                    <select
                      value={chartType}
                      onChange={(e) => setChartType(e.target.value as 'line' | 'bar' | 'combo')}
                ...
                <div className="mb-3 p-2 bg-red-50 border border-red-200 rounded-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-red-800">
                      Critical Days: {performanceData.criticalDays.length} high stock-out days
                    </span>
