...
const processRecords = (
  data: PackingPlantStockRecord[]
): PackingPlantStockRecord[] => {
  const groupedByArea = data.reduce((acc, record) => {
    if (!acc[record.area]) {
      acc[record.area] = [];
    }
...
export const usePackingPlantStockData = () => {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, date, area, opening_stock, stock_received, stock_out, closing_stock"
      );
  ...
  const addRecord = useCallback(
    async (
      record: Omit<PackingPlantStockRecord, "opening_stock" | "stock_received">
    ) => {
      ...
      try {
        ...
        if (existingRecord) {
          ...
          if (updateError) {
            ...
            console.log(
              `Successfully updated record for ${record.date} - ${record.area}`
            );
            fetchRecords();
          }
        } else {
          ...
          if (insertError) {
            ...
            console.log(
              `Successfully inserted new record for ${record.date} - ${record.area}`
            );
            fetchRecords();
          }
        }
