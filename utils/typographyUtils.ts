...
export const typographyScale = {
  ...
  responsive: {
    ...
    ui: {
      ...
      caption:
        "text-xs sm:text-sm leading-4 text-slate-500 dark:text-slate-400",
      overline:
        "text-xs sm:text-sm uppercase tracking-wider font-semibold leading-4",
    },
  },
...
export const validateContrast = (
  ...
  return (
    highContrastPairs.some(
      (highPair) => highPair[0] === pair[0] && highPair[1] === pair[1]
    ) ||
    mediumContrastPairs.some(
      (mediumPair) => mediumPair[0] === pair[0] && mediumPair[1] === pair[1]
    )
  );
};
