...
export const getAriaProps = (
  ...
  if (current !== undefined) props['aria-current'] = current ? 'page' : undefined;

  return props;
};

// Focus utilities
export const getFocusClasses = (variant: 'default' | 'inset' | 'none' = 'default'): string => {
  const variants = {
    default: 'focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2',
    inset: 'focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-inset',
