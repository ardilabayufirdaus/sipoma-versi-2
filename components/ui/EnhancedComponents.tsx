  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 rounded-inherit overflow-hidden">
        <div className="absolute inset-0 bg-white/20 scale-0 rounded-inherit transition-transform duration-300 origin-center opacity-0 hover:opacity-100 hover:scale-100" />
      </div>
    </button>
  );
};
...
export const EnhancedModal: React.FC<EnhancedModalProps> = ({
  ...
  const variantClasses = {
    ...
    glass:
      "bg-white/90 dark:bg-black/90 backdrop-blur-md border border-white/20",
    minimal:
      "bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800",
  };
...
export const useBreakpoint = (
  ...
  useEffect(() => {
    const mediaQuery = window.matchMedia(
      `(min-width: ${DESIGN_TOKENS.breakpoints[breakpoint]})`
    );
...
export const useResponsiveValue = <T,>(values: Record<string, T>) => {
  ...
  useEffect(() => {
    const mediaQueries = Object.entries(DESIGN_TOKENS.breakpoints).map(
      ([key, breakpoint]) => ({
        key,
...
export const GestaltContainer: React.FC<{
  ...
  return (
    <div className={cn(principleClasses[principle], className)}>{children}</div>
  );
};
