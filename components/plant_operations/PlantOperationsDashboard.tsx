...
const PlantOperationsDashboard: React.FC<PlantOperationsDashboardProps> = ({
  ...
  // Real data from Plant Operations untuk chart
  const realPlantOperationsData = useMemo(() => {
    // Generate hourly data based on real Plant Operations statistics
    const hoursToShow =
      timeRange === "1h"
        ? 1
        : timeRange === "4h"
    ...
    return Array.from({ length: hoursToShow }, (_, index) => {
      const time = new Date(
        currentTime.getTime() - (hoursToShow - index) * 60 * 60 * 1000
      );

      // Real metrics based on Plant Operations data
      const efficiency = Math.max(
        0,
        100 - plantOperationsStats.openDowntime * 15
      ); // Real efficiency based on open downtime
      const availability = Math.max(
        0,
        100 - plantOperationsStats.inProgressRisks * 10
      ); // Real availability based on risks
      const quality = plantOperationsStats.totalParameters > 0 ? 95 : 70; // Quality based on configured parameters
      const throughput = plantOperationsStats.totalUnits * 10; // Throughput based on plant units
      const downtime = plantOperationsStats.todayDowntime; // Real downtime hours
